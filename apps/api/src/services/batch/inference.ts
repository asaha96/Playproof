...
import type { MovementFeatures } from "@playproof/shared";
import { WoodwideClient } from "../woodwide.js";
import { sessionQueue, type QueuedSession } from "./queue.js";
import { featuresToCsv } from "../../lib/features.js";
...
const PERSISTENT_DATASET_NAME = "movement_live_inference";
...
async function runBatchInference(
  modelId: string,
  datasetName: string
): Promise<Map<string, { anomalyScore: number; isAnomaly: boolean }>> {
  // Wait a bit for dataset to be processed
  await new Promise((resolve) => setTimeout(resolve, 3000));

  // Run inference
  const results = await woodwideClient.inferAnomaly(modelId, "", true, datasetName);

  // Map results back to session IDs
  // Since we know the order, we can map by index
  const resultMap = new Map<string, { anomalyScore: number; isAnomaly: boolean }>();

  // For now, we'll need to track session order
  // In a real implementation, we'd include sessionId in the CSV and match it
  return resultMap;
}
