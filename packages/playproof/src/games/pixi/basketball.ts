...
interface BasketballGraphics {
    ball: Graphics | null;
    hoop: Graphics | null;
    backboard: Graphics | null;
...
export class BasketballGame extends PixiGameBase {
    ...
    private shotData: (ShotData & { angle: number }) | null;
    private hoopSensor: HoopSensor | null;
    private ballPassedThroughHoop: boolean;
    private ballWasAboveRim: boolean;
    private hoopX: number;
    private hoopY: number;
    private rimWidth: number;
    private rimThickness: number;
    private shootZone: ShootZone;
    ...
    constructor(gameArea: HTMLElement, config: PlayproofConfig, hooks: SDKHooks) {
        ...
        this.rimWidth = 0;
        this.rimThickness = 0;
        this.shootZone = { x: 0, y: 0, w: 0, h: 0 };
        this.scaleX = 1;
        this.scaleY = 1;
    ...
    update(dt: number): void {
        ...
        if (this.hasShot) {
            ...
            // Check if ball passes through hoop sensor
            if (this.ballWasAboveRim && !this.ballPassedThroughHoop && this.hoopSensor) {
