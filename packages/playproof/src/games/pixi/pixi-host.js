...
export class PixiHost {
    /**
     * @param {HTMLElement} mountEl - DOM element to mount canvas into
     * @param {Object} theme - PlayProof theme colors
     */
    constructor(mountEl, theme = {}) {
     ...
     * @param {Function} updateFn - Called at fixed timestep (dt in seconds)
     * @param {Function} renderFn - Called each frame with interpolation alpha
     */
     ...
     */
    createRect(x, y, width, height, color, radius = 0) {
        const g = new Graphics();
        if (radius > 0) {
     ...
     */
    createLine(x1, y1, x2, y2, color, width = 2) {
        const g = new Graphics();
        g.moveTo(x1, y1);
        g.lineTo(x2, y2);
