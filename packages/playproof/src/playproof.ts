...
export class Playproof {
  ...
  private async evaluateResult(behaviorData: BehaviorData): Promise<void> {
    ...
    if (effectiveAgentDecision) {
      ...
      const llmPassed = effectiveAgentDecision.decision === "human";
      await this.reportSession(behaviorData, suspectScore, llmPassed);
    ...
    await this.reportSession(behaviorData, score, result.passed);
   ...
   */
  private async reportSession(behaviorData: BehaviorData, suspectScore: number, llmPassed?: boolean): Promise<void> {
    try {
